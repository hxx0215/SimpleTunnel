...
class PacketTunnelProvider: NEPacketTunnelProvider, TunnelDelegate, ClientTunnelConnectionDelegate,NSNetServiceDelegate {
	...
	override func handleAppMessage(messageData: NSData, completionHandler: ((NSData?) -> Void)?) {
		...
		let responseData = "Hello luanqibazao\(messageString)".dataUsingEncoding(NSUTF8StringEncoding)
